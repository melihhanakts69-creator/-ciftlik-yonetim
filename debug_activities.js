...
const Duve = require('./backend/models/Duve');
const Buzagi = require('./backend/models/Buzagi');
const SutKaydi = require('./backend/models/SutKaydi');
...
async function debugActivities() {
    try {
        ...
        // Hardcoding connection string if .env fails, or try to load it
        // Assuming standard local mongo or uri from .env
        const MONGO_URI = process.env.MONGO_URI || 'mongodb+srv://melihhanaktas:Melih3838@cluster0.b75yq.mongodb.net/ciftlik-yonetim?retryWrites=true&w=majority&appName=Cluster0';

        await mongoose.connect(MONGO_URI);
        console.log('Connected.');

        // 2. Find a user with data
        // Fetch user with most cows to ensure data
